...
RSpec.describe RubySpriter::Processor do
  ...
  describe 'Threshold Stepping Integration (v0.7.0.1)' do
    let(:input_image) { 'spec/fixtures/test_sprite.png' }
    let(:output_image) { 'output/test_threshold.png' }
    ...
    before do
      ...
      allow(RubySpriter::Utils::FileHelper).to receive(:ensure_unique_output).and_return(output_image)
      allow(RubySpriter::Utils::FileHelper).to receive(:output_filename).and_return(output_image)
      allow(RubySpriter::Utils::OutputFormatter).to receive(:header)
      allow(RubySpriter::Utils::OutputFormatter).to receive(:success)
      allow(RubySpriter::Utils::OutputFormatter).to receive(:indent)
      allow(RubySpriter::Platform).to receive(:find_gimp).and_return('gimp')
      allow(RubySpriter::Platform).to receive(:get_gimp_version).and_return({ major: 3, minor: 0 })
    end

    describe 'edge sampling before threshold stepping' do
      it 'instantiates EdgeSampler with correct config' do
        options = {
          image: input_image,
          remove_bg: true,
        ...
        # Mock dependencies
        edge_sampler = instance_double(RubySpriter::EdgeSampler)
        allow(RubySpriter::EdgeSampler).to receive(:new).and_return(edge_sampler)
        allow(edge_sampler).to receive(:sample_edges).and_return([{ r: 255, g: 255, b: 255 }])
        allow(edge_sampler).to receive(:build_color_palette).and_return([{ r: 255, g: 255, b: 255 }])
        allow(edge_sampler).to receive(:report).and_return({ samples_collected: 100, unique_colors: 5 })
        ...
        allow(gimp_processor).to receive(:process).and_return(output_image)

        threshold_stepper = instance_double(RubySpriter::ThresholdStepper)
        allow(RubySpriter::ThresholdStepper).to receive(:new).and_return(threshold_stepper)
        allow(threshold_stepper).to receive(:process)
        allow(threshold_stepper).to receive(:report).and_return({ thresholds_processed: 6, skipped_thresholds: 0, total_time: 1.5 })

        # Verify EdgeSampler is created with correct parameters
        expect(RubySpriter::EdgeSampler).to receive(:new) do |path, config|
          expect(path).to eq(input_image)
      ...
      it 'samples edges and builds background palette' do
        options = {
          image: input_image,
          remove_bg: true,
          threshold_stepping: true
        ...
        expect(edge_sampler).to receive(:sample_edges).and_return(sample_colors)
        expect(edge_sampler).to receive(:build_color_palette).with(sample_colors).and_return(sample_colors)
        allow(edge_sampler).to receive(:report).and_return({ samples_collected: 100, unique_colors: 2 })
    ...
    describe 'background palette passed to ThresholdStepper' do
      it 'passes edge-sampled palette to ThresholdStepper' do
        options = {
          image: input_image,
          remove_bg: true,
          threshold_stepping: true
        ...
        edge_sampler = instance_double(RubySpriter::EdgeSampler)
        allow(RubySpriter::EdgeSampler).to receive(:new).and_return(edge_sampler)
        allow(edge_sampler).to receive(:sample_edges).and_return(background_palette)
        allow(edge_sampler).to receive(:build_color_palette).and_return(background_palette)
        allow(edge_sampler).to receive(:report).and_return({ samples_collected: 100, unique_colors: 3 })
        ...
        # Verify ThresholdStepper receives the background palette
        expect(RubySpriter::ThresholdStepper).to receive(:new) do |input, output, palette, gimp, opts|
          expect(palette).to eq(background_palette)
          threshold_stepper
        end
    ...
    describe 'correct processing order' do
      it 'executes: edge sampling → GIMP threshold stepping → done (no additional GIMP)' do
        options = {
          image: input_image,
          remove_bg: true,
          threshold_stepping: true
        ...
        edge_sampler = instance_double(RubySpriter::EdgeSampler)
        allow(RubySpriter::EdgeSampler).to receive(:new).and_return(edge_sampler)
        allow(edge_sampler).to receive(:sample_edges) do
          call_order << :edge_sampling
          [{ r: 255, g: 255, b: 255 }]
        end
        allow(edge_sampler).to receive(:build_color_palette).and_return([{ r: 255, g: 255, b: 255 }])
        allow(edge_sampler).to receive(:report).and_return({ samples_collected: 100, unique_colors: 1 })
        ...
        threshold_stepper = instance_double(RubySpriter::ThresholdStepper)
        allow(RubySpriter::ThresholdStepper).to receive(:new).and_return(threshold_stepper)
        allow(threshold_stepper).to receive(:process) do
          call_order << :threshold_stepping
        end
        allow(threshold_stepper).to receive(:report).and_return({ thresholds_processed: 6, skipped_thresholds: 0, total_time: 1.5 })
        ...
        # Verify order
        expect(call_order).to eq([:edge_sampling, :threshold_stepping])
      end
    end

    describe 'GIMP fuzzy select behavior' do
      xit 'skips GIMP fuzzy select when --threshold-stepping is used' do
        options = {
          image: input_image,
          remove_bg: true,
          threshold_stepping: true
        ...
        # GimpProcessor.process should NOT be called (no fuzzy select)
        expect(gimp_processor).not_to receive(:process)
    ...
    describe 'video and batch mode support' do
      xit 'supports --threshold-stepping with --video mode' do
        options = {
          video: 'input.mp4',
          remove_bg: true,
          threshold_stepping: true
        ...
        video_processor = instance_double(RubySpriter::VideoProcessor)
        allow(RubySpriter::VideoProcessor).to receive(:new).and_return(video_processor)
        allow(video_processor).to receive(:create_spritesheet).and_return('spritesheet.png')
        allow(RubySpriter::Utils::FileHelper).to receive(:validate_readable!)
        ...
        # Mock threshold stepper
        threshold_stepper = instance_double(RubySpriter::ThresholdStepper)
        allow(RubySpriter::ThresholdStepper).to receive(:new).and_return(threshold_stepper)
        allow(threshold_stepper).to receive(:process)
        allow(threshold_stepper).to receive(:report).and_return({ thresholds_processed: 6, skipped_thresholds: 0, total_time: 1.5 })

        # Verify threshold stepping is applied to video output
        expect(RubySpriter::ThresholdStepper).to receive(:new)

        processor.run
      end

      xit 'supports --threshold-stepping with --batch mode' do
        options = {
          batch: true,
          dir: 'videos/',
          remove_bg: true,
        ...
        batch_processor = instance_double(RubySpriter::BatchProcessor)
        allow(RubySpriter::BatchProcessor).to receive(:new).and_return(batch_processor)
        allow(batch_processor).to receive(:process).and_return({ processed: 1, failed: 0 })

        # Batch processor should receive options with threshold_stepping
        expect(RubySpriter::BatchProcessor).to receive(:new) do |opts|
          expect(opts[:threshold_stepping]).to be true
          batch_processor
        end
